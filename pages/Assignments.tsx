...
interface AssignmentsPageProps {
  ...
  onSync: (assignment: Assignment) => void;
  onSyncDelete: (id: string) => void;
}

export const AssignmentsPage: React.FC<AssignmentsPageProps> = ({ t, assignments, setAssignments, users, facilities, forms, menus, facilityTypes, onTabChange, onSync, onSyncDelete }) => {
  ...
  const handleSave = () => {
    ...
    if (editingAssignment) {
      ...
      if (newAssignments.length > 0) {
        ...
        // Call sync for each new assignment
        newAssignments.forEach(onSync);
        showAlert(
          skipCount > 0 
  ...
  const confirmDelete = () => {
    if (assignmentToDelete) {
      ...
      onSyncDelete(assignmentToDelete.id);
  ...
  const confirmBulkDelete = () => {
    if (bulkDeleteGroup) {
      ...
      // Call sync delete for each
      toDelete.forEach(a => onSyncDelete(a.id));
