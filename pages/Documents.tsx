...
export const DocumentsPage: React.FC<DocumentsPageProps> = ({ t, documents, setDocuments, onSync, onSyncDelete, onLog }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 text-left pb-20">
      <header>
        <h1 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Dokumentenverwaltung</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">Laden Sie Sicherheitsunterweisungen und Handbücher für Ihre Mitarbeiter hoch.</p>
      </header>
      ...
      <div className="bg-white dark:bg-slate-900 p-8 rounded-[3.5rem] border border-slate-100 dark:border-slate-800 shadow-sm">
        <h2 className="text-lg font-black text-slate-900 dark:text-white uppercase mb-6 flex items-center gap-2">
          <span>📤</span> Neues Dokument hochladen
        </h2>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-end">
          <div className="space-y-2">
            ...
            <input 
              ...
              placeholder="z.B. Brandschutzhelfer Anweisung"
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex bg-slate-100 dark:bg-slate-800 p-1.5 rounded-2xl overflow-hidden">
               <button 
                onClick={() => setDocCategory('safety')}
                title="Sicherheit"
                className={`flex-1 py-3 px-2 rounded-xl text-[10px] font-black uppercase transition-all flex items-center justify-center gap-2 ${docCategory === 'safety' ? 'bg-white dark:bg-slate-700 text-blue-600 shadow-sm' : 'text-slate-500'}`}
               >
                 <span>🛡️</span> <span className="hidden sm:inline">Sicherheit</span>
               </button>
               <button 
                onClick={() => setDocCategory('staff')}
                title="Personal"
                className={`flex-1 py-3 px-2 rounded-xl text-[10px] font-black uppercase transition-all flex items-center justify-center gap-2 ${docCategory === 'staff' ? 'bg-white dark:bg-slate-700 text-indigo-600 shadow-sm' : 'text-slate-500'}`}
               >
                 <span>👤</span> <span className="hidden sm:inline">Personal</span>
               </button>
               <button 
                onClick={() => setDocCategory('hygiene')}
                title="Hygiene"
                className={`flex-1 py-3 px-2 rounded-xl text-[10px] font-black uppercase transition-all flex items-center justify-center gap-2 ${docCategory === 'hygiene' ? 'bg-white dark:bg-slate-700 text-emerald-600 shadow-sm' : 'text-slate-500'}`}
               >
                 <span>🧼</span> <span className="hidden sm:inline">Hygiene</span>
               </button>
            </div>
          </div>

          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">PDF Datei auswählen</label>
            <div className="flex gap-4">
              ...
              <button 
                ...
                className={`px-8 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl transition-all ${!selectedFile || !docTitle.trim() || isUploading ? 'bg-slate-200 text-slate-400' : 'bg-blue-600 text-white hover:bg-blue-700'}`}
              >
                {isUploading ? '...' : 'Upload'}
              </button>
            </div>
          ...
          <div key={doc.id} className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm group hover:shadow-xl transition-all relative overflow-hidden">
             <div className="flex justify-between items-start mb-6">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-3xl shadow-inner ${
                  doc.category === 'hygiene' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-900/30' : 
                  doc.category === 'staff' ? 'bg-indigo-50 text-indigo-600 dark:bg-indigo-900/30' :
                  'bg-blue-50 text-blue-600 dark:bg-blue-900/30'
                }`}>
                  {getCategoryIcon(doc.category)}
                </div>
                <div className="flex flex-col gap-2">
                   <button onClick={() => setPreviewDoc(doc)} className="p-2.5 bg-slate-50 dark:bg-slate-800 text-blue-600 rounded-xl hover:scale-110 transition-transform">👁️</button>
                   <button onClick={() => handleDelete(doc.id, doc.title)} className="p-2.5 bg-rose-50 dark:bg-rose-900/20 text-rose-600 rounded-xl hover:scale-110 transition-transform">🗑️</button>
                </div>
             </div>
             ...
             <div className="flex items-center gap-2 mb-6">
                <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest border ${getCategoryColorClass(doc.category)}`}>
                  {doc.category === 'hygiene' ? 'HYGIENE' : doc.category === 'staff' ? 'PERSONAL' : 'SICHERHEIT'}
                </span>
             </div>
             <div className="pt-6 border-t border-slate-50 dark:border-slate-800">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Hochgeladen: {new Date(doc.createdAt).toLocaleDateString('de-DE')}</span>
             </div>
          </div>
        ))}
        {documents.length === 0 && (
          <div className="col-span-full py-20 text-center text-slate-400 font-bold uppercase italic border-2 border-dashed rounded-[3rem]">Keine Dokumente vorhanden</div>
        )}
      </div>
      ...
      {previewDoc && (
        <div className="fixed inset-0 z-[2000] bg-slate-950/90 backdrop-blur-xl flex flex-col p-4 md:p-10 animate-in fade-in duration-300">
          <div className="flex justify-between items-center mb-6 text-white max-w-7xl mx-auto w-full">
            ...
            <button onClick={() => setPreviewDoc(null)} className="w-12 h-12 bg-white/10 rounded-2xl flex items-center justify-center text-2xl font-bold hover:bg-white/20 transition-all">✕</button>
          </div>
          <div className="flex-1 w-full max-w-7xl mx-auto bg-white rounded-[2rem] overflow-hidden shadow-2xl">
