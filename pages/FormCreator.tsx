...
export const FormCreatorPage: React.FC<FormCreatorPageProps> = ({ t, forms, setForms, onSync, onSyncDelete }) => {
  ...
  const loadSupervisorTemplate = () => {
    setBuilderTitle('Supervisor Vor-Ort Check');
    setBuilderQuestions([
      { id: Date.now().toString(), text: 'Hat der Supervisor den Standort heute besucht?', type: 'yesno', options: [] }
    ]);
    setBuilderSignature(true);
    showAlert("Supervisor-Vorlage geladen.", "success");
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-500 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 text-left">
        ...
        <div className="flex gap-3">
           <button 
             onClick={loadSupervisorTemplate}
             className="px-6 py-3 bg-indigo-50 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400 rounded-2xl font-black text-[10px] uppercase tracking-widest border border-indigo-100 hover:bg-indigo-100 transition-all"
           >
             Supervisor Vorlage
           </button>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-in fade-in duration-300">
          <div className="bg-white dark:bg-slate-900 w-full max-w-4xl max-h-[90vh] rounded-[3rem] shadow-2xl flex flex-col overflow-hidden border border-white/10 text-left relative">
            <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50">
              <h3 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tighter">{editingForm ? 'Formular bearbeiten' : 'Formular-Designer'}</h3>
              <div className="flex items-center space-x-2">
                 {!editingForm && (
                    <button onClick={loadSupervisorTemplate} className="px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl text-[10px] font-black uppercase">Vorlage</button>
                 )}
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar pb-32">
              ...
              <div className="space-y-6">
                ...
                {builderQuestions.map((q, idx) => (
                  <div key={q.id} className="p-8 rounded-[2.5rem] bg-slate-50 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800 space-y-6 relative group/q animate-in slide-in-from-top-2">
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        <div className="flex space-x-2">
                           {(['text', 'choice', 'yesno'] as QuestionType[]).map(type => (
                             <button key={type} onClick={() => updateQuestion(q.id, { type })} className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border transition-all ${q.type === type ? 'bg-blue-600 border-blue-600 text-white shadow-md' : 'bg-white dark:bg-slate-700 text-slate-400'}`}>
              ...
              <div className="p-8 rounded-[2.5rem] bg-indigo-50/50 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-800 flex items-center justify-between">
                 <div className="flex items-center space-x-4">
                    ...
                    <div>
                       <p className="font-black text-slate-900 dark:text-white uppercase tracking-tight leading-none mb-1">Digitale Signatur erzwingen</p>
                       <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Muss zum Abschluss signiert werden</p>
                    </div>
                 </div>
                 <button 
            ...
            <div className="p-8 border-t border-slate-100 dark:border-slate-800 flex justify-end space-x-4 bg-slate-50/50 dark:bg-slate-800/50">
              <button onClick={() => setIsBuilderOpen(false)} className="px-6 py-3 text-slate-500 font-black uppercase text-[10px] tracking-widest">Abbrechen</button>
              <button onClick={saveForm} className="bg-blue-600 text-white px-10 py-3 rounded-2xl font-black shadow-xl hover:scale-105 transition-transform uppercase text-[10px] tracking-widest">Speichern</button>
            </div>
          </div>
        </div>
      )}
      ...
      {previewForm && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 z-[100] animate-in fade-in duration-300">
           <div className="bg-white dark:bg-slate-900 w-full max-w-xl rounded-[3rem] shadow-2xl flex flex-col overflow-hidden">
              <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                 <h3 className="text-xl font-black uppercase tracking-tight">Formular Vorschau</h3>
                 <button onClick={() => setPreviewForm(null)} className="w-10 h-10 flex items-center justify-center rounded-xl bg-slate-200 text-slate-500">✕</button>
              </div>
              <div className="p-10 space-y-8 overflow-y-auto max-h-[60vh] custom-scrollbar">
                 <h2 className="text-2xl font-black text-slate-900 dark:text-white text-center mb-10">{previewForm.title}</h2>
                 {previewForm.questions.map((q, idx) => (
                    <div key={q.id} className="space-y-3">
                       <p className="font-bold text-slate-800 dark:text-slate-200">{idx+1}. {q.text}</p>
                       <div className="h-12 w-full bg-slate-50 dark:bg-slate-800 border rounded-xl" />
                    </div>
                 ))}
                 {previewForm.requiresSignature && (
                    <div className="mt-10 pt-10 border-t border-slate-100">
                       <p className="text-[10px] font-black uppercase text-slate-400 mb-4">Digitale Unterschrift</p>
                       <div className="h-32 w-full bg-slate-50 dark:bg-slate-800 border border-dashed rounded-2xl" />
                    </div>
                 )}
              </div>
           </div>
