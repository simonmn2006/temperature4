...
export const UsersPage: React.FC<UsersPageProps> = ({ t, currentUser, users, setUsers, facilities, onLog, onSync }) => {
  ...
  const handleSave = () => {
    ...
    // Clear email if role is standard user before saving
    const finalEmail = formData.role === 'User' ? undefined : formData.email;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 text-left">
        <div>
          <h1 className="text-2xl font-black text-slate-900 dark:text-white uppercase">{t.tabs.users}</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">Verwaltung von Mitarbeitern und Management</p>
        </div>
        ...
        <table className="w-full text-left">
          <thead className="bg-slate-50 dark:bg-slate-800/50">
            <tr>
              ...
              <th className="px-8 py-4 text-[11px] font-black text-slate-400 uppercase tracking-widest">Verwaltung</th>
            ...
            {filteredUsers.map((user) => (
              <tr key={user.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                <td className="px-8 py-6">
                  ...
                  <span className="text-[10px] font-black text-blue-600 uppercase">@{user.username} • {user.role}</span>
                  {user.email && isPrivilegedRole(user.role) && (
                ...
                <td className="px-8 py-6">
                  <div className="flex flex-wrap gap-1">
                     <span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 rounded text-[9px] font-bold text-slate-600 dark:text-slate-400 uppercase">
                       Base: {facilities.find(f => f.id === user.facilityId)?.name || 'Global'}
                     </span>
            ...
            <div className="space-y-6 overflow-y-auto max-h-[65vh] pr-2 custom-scrollbar">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className={`w-full px-5 py-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border font-bold text-sm outline-none ${invalidFields.has('name') ? 'border-rose-500' : 'border-slate-200'}`} />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase mb-2 px-1">Rolle</label>
                  <select disabled={isFieldDisabled('role')} value={formData.role} onChange={e => setFormData({...formData, role: e.target.value as any})} className="w-full px-5 py-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 font-bold text-sm outline-none">
              ...
              {isPrivilegedRole(formData.role) && (
                <div className="animate-in slide-in-from-top-2">
                  <label className="block text-[10px] font-black text-slate-400 uppercase mb-2 px-1">E-Mail Adresse (für Alarme)</label>
                  <input 
                    type="email" 
                    ...
                    placeholder="beispiel@gourmetta.de" 
                  />
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-4">
                 <div>
                   ...
                   <input type="text" disabled={isFieldDisabled('username')} value={formData.username} onChange={e => setFormData({...formData, username: e.target.value})} className="w-full px-5 py-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 font-bold text-sm outline-none disabled:opacity-50" />
                 </div>
                 <div>
                   <label className="block text-[10px] font-black text-slate-400 uppercase mb-2 px-1">Passwort</label>
                   <input type="password" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} className="w-full px-5 py-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 font-bold text-sm outline-none" placeholder="••••••••" />
                 </div>
              </div>

              <div className="relative" ref={dropdownRef}>
                <label className="block text-[10px] font-black text-slate-400 uppercase mb-2 px-1">Heimat-Standort (Primär)</label>
                ...
                {isDropdownOpen && !isFieldDisabled('facilityId') && (
                  <div className="absolute top-full left-0 w-full mt-2 bg-white dark:bg-slate-900 border border-slate-100 rounded-2xl shadow-xl z-50 max-h-48 overflow-y-auto">
                    <button onClick={() => { setFormData({...formData, facilityId: ''}); setFacilitySearch('Kein Standort'); setIsDropdownOpen(false); }} className="w-full text-left px-5 py-3 hover:bg-slate-50 font-bold text-sm border-b">Kein Standort</button>
              ...
              {formData.role === 'Manager' && (
                <div className="p-6 bg-blue-50/50 dark:bg-blue-900/10 rounded-[2rem] border border-blue-100 dark:border-blue-800 space-y-4 animate-in slide-in-from-top-2">
                  <label className="block text-[10px] font-black text-blue-600 uppercase tracking-widest px-1">Zusätzliche verwaltete Standorte</label>
                  ...
                  <div className="relative" ref={managedDropdownRef}>
                    ...
                    {isManagedDropdownOpen && managedFilteredFacilities.length > 0 && (
                      <div className="absolute top-full left-0 w-full mt-2 bg-white dark:bg-slate-900 border border-slate-100 rounded-xl shadow-xl z-[60] max-h-48 overflow-y-auto">
                        {managedFilteredFacilities.map(f => (
                          <button key={f.id} onClick={() => { setFormData({...formData, managedFacilityIds: [...(formData.managedFacilityIds || []), f.id]}); setIsManagedDropdownOpen(false); setManagedSearch(''); }} className="w-full text-left px-5 py-3 hover:bg-slate-50 font-bold text-xs uppercase border-b last:border-0 border-slate-50 dark:border-slate-800">{f.name}</button>
                        ))}
                      </div>
                    )}
                  </div>
