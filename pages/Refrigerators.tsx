...
interface RefrigeratorsPageProps {
  ...
  onSync: (fridge: Refrigerator) => void;
...
export const RefrigeratorsPage: React.FC<RefrigeratorsPageProps> = ({ 
  ...
  const handleOpenEdit = (fridge: Refrigerator) => {
    ...
    const type = fridgeTypes.find(t => t.id === (fridge as any).typeId || t.name === (fridge as any).typeName);
  ...
  const handleSave = () => {
    ...
    if (editingFridge) {
      ...
      const updatedFridge: Refrigerator = { 
        ...editingFridge, 
        name: row.name,
        facilityId: modalFacilityId,
        typeName: typeName
      };
      setFridges(prev => prev.map(f => f.id === editingFridge.id ? updatedFridge : f));
      // Call sync
      onSync(updatedFridge);
      ...
      // Call sync for each new fridge
      additions.forEach(onSync);
  ...
  const confirmDeleteFridge = () => {
    if (fridgeToDelete) {
       ...
       // Call sync delete
       onSyncDelete(fridgeToDelete.id);
        ...
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4 z-[200] animate-in fade-in duration-300">
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-[4rem] p-12 shadow-2xl border border-rose-500/30 text-center relative overflow-hidden">
            ...
            <div className="flex flex-col space-y-4">
              <button onClick={confirmDeleteFacility} className="w-full bg-rose-600 text-white font-black py-5 rounded-[1.75rem] shadow-xl transition-all uppercase text-sm tracking-widest">UNWIDERRUFLUICH LÖSCHEN</button>
